...
import { getUserRole, isAdmin, isStaff } from '../../utils/roleUtils';
...
describe('roleUtils', () => {
    describe('getUserRole', () => {
        ...
        it('should extract non-privileged role from user_metadata if app_metadata is missing', () => {
            const user = { app_metadata: {}, user_metadata: { role: 'vip' } } as unknown as User;
            expect(getUserRole(user)).toBe('vip');
        });

        it('should reject staff or admin roles from user_metadata for security', () => {
            const adminUser = { app_metadata: {}, user_metadata: { role: 'admin' } } as unknown as User;
            expect(getUserRole(adminUser)).toBeNull();
            expect(isAdmin(adminUser)).toBe(false);

            const staffUser = { app_metadata: {}, user_metadata: { role: 'staff' } } as unknown as User;
            expect(getUserRole(staffUser)).toBeNull();
            expect(isStaff(staffUser)).toBe(false);
        });
