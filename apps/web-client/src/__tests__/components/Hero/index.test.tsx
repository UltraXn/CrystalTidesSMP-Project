...
describe("Hero (index)", () => {
  ...
  it("renders external and internal navigation links", () => {
    renderWithProviders(
      <Hero mockIsOnline={true} mockPlayerCount={5} mockSlides={[]} />
    );

    const kofiLink = screen.getByRole("link", { name: /ko-fi/i });
    expect(kofiLink).toHaveAttribute("href", "https://ko-fi.com/ultraxn");

    const donorsLink = screen.getByRole("link", { name: /donadores/i });
    expect(donorsLink).toHaveAttribute("href", "/donors");
  });
});
