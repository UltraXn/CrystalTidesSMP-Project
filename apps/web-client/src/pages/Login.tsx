            ...
            <div className="w-full max-w-md bg-white/5 border border-white/10 backdrop-blur-2xl rounded-3xl p-8 shadow-2xl relative overflow-hidden text-left">
                ...
                <div className="text-center mb-8">
                    ...
                    <p className="text-xs text-gray-400 mt-1 font-medium">
                        {t('login.welcome', 'Bienvenido de nuevo, aventurero')}
                    </p>
                </div>
                ...
                <form className="space-y-5" onSubmit={handleSubmit(onSubmit)}>
                    <div className="space-y-1.5">
                        <label htmlFor="login-email" className="flex items-center gap-2 text-xs font-bold text-gray-300 uppercase tracking-wider">
                            <Mail className="w-4 h-4 text-(--accent)" /> {t('login.email_label', 'Correo Electrónico')}
                        </label>
                        <input id="login-email"
                            type="email"
                            placeholder={t('login.email_placeholder', 'correo@ejemplo.com')}
                    ...
                    <div className="space-y-1.5">
                        <label htmlFor="login-password" className="flex items-center gap-2 text-xs font-bold text-gray-300 uppercase tracking-wider">
                            <Lock className="w-4 h-4 text-(--accent)" /> {t('login.password_label', 'Contraseña')}
                        </label>
                    ...
                    <div className="text-right">
                        <Link to="/forgot-password" className="text-xs text-gray-400 hover:text-(--accent) transition-colors font-medium">
                    ...
                    >
                        {isSubmitting ? (
                            <span>{t('login.loading', 'Ingresando...')}</span>
                        ) : (
                            <>
                                <LogIn className="w-4 h-4" />
                                <span>{t('login.sign_in_verb', 'Iniciar Sesión')}</span>
                            </>
                        )}
                    </button>
                </form>
                ...
                <div className="flex items-center gap-3 my-6">
                    ...
                    <span className="text-xs font-bold uppercase text-gray-500 tracking-wider">
                        {t('login.or_sign_in_with', 'O ingresa con')}
                    </span>
                    <div className="flex-1 h-px bg-white/10" />
                </div>
                ...
                <div className="pt-4 border-t border-white/10 text-center space-y-2">
                    ...
                    <Link to="/" className="inline-block text-xs text-gray-500 hover:text-white transition-colors pt-2">
                        ← {t('login.back_home', 'Volver al inicio')}
                    </Link>
                </div>
            </div>
        </div>
