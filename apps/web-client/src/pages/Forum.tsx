...
export default function Forum() {
    ...
    useSEO({
        title: 'Foro Oficial de la Comunidad',
        description: 'Participa en los debates, comparte tus construcciones e historias y conéctate con los jugadores en el foro de CrystalTides SMP.',
        keywords: 'foro minecraft, comunidad smp, debate minecraft, historias minecraft, foro espanol',
        canonical: 'https://crystaltidessmp.net/forum'
    });
    const API_URL = import.meta.env.VITE_API_URL

    const { data: activePoll = null } = useQuery<Poll | null>({
        queryKey: ['forumActivePoll'],
        queryFn: async () => {
            ...
            if (poll && !poll.message && poll.options && poll.options.length > 0) return poll
            return null
        },
    ...
    const { data: statsData } = useQuery<CategoryStat[]>({
        queryKey: ['forumStats'],
        queryFn: async () => {
            if (!API_URL) return []
    ...
    const categories: Category[] = useMemo(() => {
        if (!statsData || !Array.isArray(statsData)) return initialCategories
        return initialCategories.map(cat => {
            const stat = statsData.find(s => s.id === cat.id)
            return stat ? { ...cat, topics: stat.topics, posts: stat.posts, lastPost: stat.lastPost } : cat
        })
    }, [statsData])

    const { data: latestNews = null } = useQuery<NewsItem | null>({
        queryKey: ['forumLatestNews'],
        queryFn: async () => {
            if (!API_URL) return null
                                    ...
                                    {(activePoll.options || []).slice(0, 3).map((opt: PollOption) => (
                                        <div key={opt.id} className="relative group">
                                            <div className="h-12 bg-white/5 rounded-xl overflow-hidden relative">
                                                <div 
                                                    className="absolute inset-y-0 left-0 bg-(--accent) opacity-20 transition-colors duration-1000 ease-out group-hover:opacity-30" 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-6xl mx-auto">
                    {categories.map((cat) => (
                        <Link 
                            to={`/forum/${cat.slug}`} 
                            key={cat.id} 
                            className="bg-white/5 backdrop-blur-md border border-white/5 rounded-2xl p-6 flex flex-col sm:flex-row items-center sm:items-stretch gap-6 transition-colors duration-300 hover:bg-white/10 hover:border-(--accent)/40 hover:-translate-y-1 hover:shadow-xl hover:shadow-(--accent)/5 group"
                        >
                            {/* Icon Box */}
                            <div className="w-16 h-16 sm:w-20 sm:h-auto bg-black/40 rounded-xl flex items-center justify-center text-3xl text-gray-400 group-hover:text-(--accent) group-hover:bg-(--accent)/10 transition-colors shrink-0">
                                {cat.icon}
                            </div>
                            ...
                            <div className="flex-1 text-center sm:text-left min-w-0">
                             ...
                             <div className="hidden xl:flex flex-col justify-center items-end text-right min-w-30 text-xs text-gray-500 border-l border-white/5 pl-6 ml-2">
