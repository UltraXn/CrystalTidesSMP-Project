...
export const GachaControls: React.FC<GachaControlsProps> = ({
    ...
    return (
        <div className={`slot-controls tier-${selectedTier.id}`} id="gacha_controls">
            ...
            <div role="status" aria-live="polite" className="gacha-balance-pill border border-amber-500/30 bg-amber-500/10 backdrop-blur-md px-5 py-2.5 rounded-2xl flex items-center justify-between gap-3 mb-4 w-full shadow-lg">
                <div className="flex items-center gap-2 text-amber-400 font-black uppercase text-xs tracking-wider">
                    <img src="/images/killucoin.png" alt="" aria-hidden="true" className="w-5 h-5 object-contain [image-rendering:pixelated] animate-pulse" />
                    <span>{t('gacha.balance_label', 'Saldo Disponible')}</span>
                </div>
                <div className="flex items-center gap-1.5 font-mono font-extrabold text-amber-300 text-sm tabular-nums">
            ...
            <div className="flex gap-3 w-full">
                <button type="button"
                    id="gacha_spin1"
                    aria-busy={isOpening}
                    aria-label={isOpening ? t('gacha.spinning') : `${t('gacha.spin')} x1 (${formatCost(cost1)} KC)`}
                ...
                <button type="button"
                    id="gacha_spin10"
                    aria-busy={isOpening}
                    aria-label={isOpening ? t('gacha.spinning') : `${t('gacha.spin')} x10 (${formatCost(cost10)} KC)`}
