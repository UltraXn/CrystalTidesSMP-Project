...
export const WikiCompanion3DCard: React.FC<WikiCompanion3DCardProps> = ({
    minimal3dOnly = false,
    modelPath = '/models/critters/red_panda.gltf',
    bossName = 'Red Panda',
    ...
    useEffect(() => {
        ...
        emeraldLight.position.set(3, 5, 4);
        scene.add(emeraldLight);

        const cyanFill = new THREE.PointLight(0x06b6d4, 1.5, 10);
        cyanFill.position.set(-3, 2, -2);
        scene.add(cyanFill);
    ...
    if (minimal3dOnly) {
        return (
            <div className="relative w-full h-full min-h-80 bg-radial from-emerald-950/20 via-black to-black flex items-center justify-center cursor-grab active:cursor-grabbing overflow-hidden border border-emerald-500/20 rounded-2xl">
                {isLoading && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/80 backdrop-blur-xs z-20">
                        ...
                        <span className="text-[9px] uppercase tracking-widest text-emerald-400 font-mono">Cargando 3D...</span>
                    </div>
                )}
    ...
    return (
        <div className="w-full my-1">
            <div className="relative overflow-hidden rounded-2xl border border-emerald-500/20 bg-[#06100c]/90 shadow-xl backdrop-blur-xl">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-0 items-stretch">
                    {/* 3D Viewport Column */}
                    <div
                        tabIndex={0}
                        role="region"
                        aria-label="Visualizador 3D Mascota"
                        className="lg:col-span-5 relative w-full h-full min-h-95 bg-radial from-emerald-950/30 via-black to-black flex items-center justify-center cursor-grab active:cursor-grabbing border-b lg:border-b-0 lg:border-r border-emerald-500/20 overflow-hidden outline-none"
                    ...
                    <div className="lg:col-span-7 p-5 flex flex-col justify-between space-y-3">
                        <div>
                            <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-[9px] font-black uppercase tracking-widest mb-1.5">
                                <Heart className="w-3 h-3" /> Critters & Companions • {subtitle}
                            </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-3">
                                <div className="p-2.5 rounded-xl bg-white/5 border border-white/5 flex items-center gap-2.5">
                                    ...
                                    <div>
                                        <div className="text-[8px] text-gray-400 uppercase font-mono tracking-wider">Domesticación</div>
                                ...
                                <div className="p-2.5 rounded-xl bg-white/5 border border-white/5 flex items-center gap-2.5">
                                    ...
                                    <div>
                                        <div className="text-[8px] text-gray-400 uppercase font-mono tracking-wider">Temperamento</div>
                                ...
                                <div className="p-2.5 rounded-xl bg-white/5 border border-white/5 flex items-center gap-2.5">
                                    ...
                                    <div>
                                        <div className="text-[8px] text-gray-400 uppercase font-mono tracking-wider">Salud de Mascota</div>
                                        <div className="text-[11px] font-bold text-white">
                                            {hp}
                                        </div>
                                    </div>
                                ...
                                <div className="p-2.5 rounded-xl bg-white/5 border border-white/5 flex items-center gap-2.5">
                                    ...
                                    <div>
                                        <div className="text-[8px] text-gray-400 uppercase font-mono tracking-wider">Marcha & Agilidad</div>
                                        <div className="text-[11px] font-bold text-white">
                                            {speed}
                                        </div>
                                    </div>
                            ...
                            {/* Companion Trait */}
                            <div className="p-2.5 rounded-xl bg-emerald-500/5 border border-emerald-500/10 space-y-1 mb-2">
                                <div className="flex items-center gap-1.5 text-[11px] font-bold text-emerald-400 uppercase tracking-wider">
                                    <Sparkles size={12} /> Beneficio de Mascota
                                </div>
                                <p className="text-[10px] text-gray-300 font-medium leading-tight">
                                    <strong className="text-white">Domesticable:</strong> Al alimentarlo con su comida favorita, este compañero te seguirá fielmente por CrystalTides.
                                </p>
                            </div>
                            ...
                            <div className="p-2.5 rounded-xl bg-white/5 border border-white/5 space-y-1 mb-2">
                                <div className="flex items-center gap-1.5 text-[11px] font-bold text-cyan-400 uppercase tracking-wider">
                                    <MapPin size={12} /> Hábitat Natural
                                </div>
                                <p className="text-[10px] text-gray-300 font-medium leading-tight">
                            ...
                            <div>
                                <h4 className="text-[10px] font-bold text-white uppercase tracking-wider mb-1 flex items-center gap-1.5">
                                    <Gift className="w-3.5 h-3.5 text-emerald-400" /> Drops & Recompensas
                                </h4>
                                <div className="flex flex-wrap gap-1.5">
                                    {drops.map((drop) => (
                                        <span key={drop} className="px-2 py-0.5 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-200 text-[10px] font-semibold">
                                            🐾 {drop}
                                        </span>
                                    ))}
                                </div>
...
export default WikiCompanion3DCard;
